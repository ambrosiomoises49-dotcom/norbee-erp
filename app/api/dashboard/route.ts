...
export async function GET() {
  ...
  try {
    ...
    const merchandiseCost = monthSales.reduce((saleSum, sale) => {

      const itemCost = sale.items.reduce(

        (itemSum, item) =>

          itemSum + Number(item.unitCost || 0) * Number(item.quantity || 0),
    ...
    const stockValue = centralStocks.reduce(
      ...
      (sum, stock) =>
        ...
        sum + Number(stock.quantity || 0) * Number(stock.avgCost || 0),

      0

    );

    const potentialProfit = centralStocks.reduce((sum, stock) => {
    ...
    const monthlyChart = Array.from({ length: 12 }, (_, index) => {
      ...
      const label = new Date(now.getFullYear(), index).toLocaleString("pt-PT", {
